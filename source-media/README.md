...
## `dances-gena/` — Gena's 16 dance clips

Generated by an AI video model from green-screen stills of her four tier
sprites (`tools/pad-for-video.mjs` makes those), delivered as 1072x1920 H.264
at 30fps with a flat `#00FF00` background and a generator watermark in the
corner. Unlike Jimmy's, the background is one colour, so
`pipeline/key_green.py` is a single green-dominance cut (measured: the
background never falls below ~208 and she has no green on her), plus a
despill for the H.264 chroma bleed on the rim; the watermark goes as a
sub-2% island, the same way it does for him.

`pipeline/build_gena.py` re-anchors every clip onto the **same canvas as
Jimmy's** — pinned to `canvas.json` (376x660, ground line at 554, character
491 tall) rather than recomputed from the clips — so `JimmyAnimation` swaps
her sprite for her dance for exactly the reason it works for him. Output is
`public/animations/dances/gena/dance{N}/stage{N}.webp` plus the store-card
`preview/` set, in the same layout as his.

```bash
python3 source-media/pipeline/build_gena.py            # all 16 + previews
python3 source-media/pipeline/build_gena.py --only 1/1 # one clip
```
