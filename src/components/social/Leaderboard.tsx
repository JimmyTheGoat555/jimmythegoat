...
import { lifetimeVolume, weeklySummary } from '../../utils/workoutStats';
...
// Derives each FOLLOWED friend's weekly tonnage from their own recent feed
// posts (see hooks/useFeed.js) rather than a live query against their
// private workout history — a friend's users/{uid}/workouts subcollection
// is only readable by its owner or a connected trainer (see
...
// (userName, totalVolume, timestamp) and are universally readable by
// design, so this is the same data a friend would see in their own feed,
// just re-aggregated. A friend with no posts in the last 7 days simply
// doesn't show up here — same as them having 0kg this week.
function weeklyFriendTotals(feedPosts: FeedPost[]): Rankable[] {
  ...
  for (const post of feedPosts) {
    ...
    if (existing) {
      existing.weeklyTonnage += post.totalVolume;
    } else {
      ...
      // this value.
      totals.set(post.userId, { id: post.userId, username: post.userName, lifetimeVolume: 0, weeklyTonnage: post.totalVolume, isYou: false });
    }
  }
    ...
    <div className="card flex items-center gap-3 p-4">
      ...
      <div className="text-right shrink-0">
        <p className="text-base font-semibold text-neutral-100 tabular-nums">
          {entry.weeklyTonnage.toLocaleString('en-US')}
        </p>
        <p className="text-xs text-neutral-500">kg this wk</p>
      </div>
    </div>
  );
