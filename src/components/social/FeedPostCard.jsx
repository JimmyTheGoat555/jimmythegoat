...
import { getEvolutionProgress, progressionScale } from '../../utils/evolutionTiers';
...
export default function FeedPostCard({ post, myUid }) {
  ...
  const dance = post.equippedDance && mascotHasDances(mascot) ? ITEMS_BY_ID.get(post.equippedDance) : null;
  ...
  // The ladder rides on the post too (logWorkout stamps progressionScale
  // beside minStage); older posts fall back to the mascot they carry.
  const postTier = getEvolutionProgress(post.lifetimeVolume ?? 0, {
    minStage: Number(post.minStage) || 1,
    scale: progressionScale(post),
  }).current;
  ...
  return (
    <div className="card p-4 flex flex-col gap-2" style={heat.style}>
      <div className="flex items-center gap-2">
        ...
        {/* The equipped-dance emoji — hidden with the rest of the emotes
            while ENABLE_EMOTES (config/features.js) is off. */}
        {ENABLE_EMOTES && dance && (
      ...
      <p className="text-base text-neutral-200">
        ...
        <span className={`font-semibold tabular-nums ${heat.tag}`}>{post.totalVolume.toLocaleString('en-US')} kg</span>
        {heat.label && (
