...
import { progressionScale } from '../../utils/evolutionTiers';
import { useFriendSuggestions } from '../../hooks/useFriendSuggestions';
import { useFriendSummaries } from '../../hooks/useFriendSummaries';
...
export default function SocialPage({
  ...
  // Every friend's public summary, for the board's rows that have no post
  // this week. Held back until the board tab is open — the page lands on
  // the feed, and most visits never leave it.
  const { summaries: friendSummaries, loading: summariesLoading } = useFriendSummaries(
    friendUids,
  ...
  return (
    <div className="flex flex-col gap-5 pt-6 pb-nav">
      ...
      <JimmyWelcomeBanner myUid={myUid} friendUids={friendUids} officialFriendUid={officialFriendUid} />
      ...
      {activeTab === 'leaderboard' ? (
        <Leaderboard
          ...
          progressionScale={progressionScale(account)}
          // Your row alone — every other row reads its mascot off their
          // public summary or the feed post that produced it.
          mascot={resolveMascotId(account)}
          // The whole friends list, so the board shows everyone — not just
          // whoever posted this week.
          friends={friends}
